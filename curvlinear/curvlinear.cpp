...
void SectionOptimization(TableOfCurvilinearRegions &TableauListesPointsFiltrees, int Larg, int Haut)
{
   ...
   DPoint2D *pPosInit, *pPosTemp, *pPosFinal;
   DPoint2D GravInit, PointTwo, PointTwoPerp;

   pLocalLine = new PosDetectCurvi[LengthLocalLine];
   pPosInit   = new DPoint2D[LengthLocalLine];
   pPosTemp   = new DPoint2D[LengthLocalLine];
   pPosFinal  = new DPoint2D[LengthLocalLine];

   STabIndice *TabR    = new STabIndice[LengthLocalLine];
   STabIndice *TabRInv = new STabIndice[LengthLocalLine];

   double VarInit,CovInit;
   double Dist, DistMin;
   int IndiceMin;
   IPoint2D * TabCoupe = new IPoint2D[Haut+Larg];
   ...
   for (TableOfCurvilinearRegions::iterator it = TableauListesPointsFiltrees.begin(); it != TableauListesPointsFiltrees.end(); it++)// pour toutes les régions
   {
      ...
      while (pos != (*it)->end())
      {
         ...
         if (!fin)
         {
            ...
            if (pos != (*it)->end()) 
            {
               for (i=0; i<LengthLocalLine; i++) // rewind the list if we are not at the end
                  pos--;                  
            }
            ...
            for (i=0;i<LengthLocalLine;i++)
            {
               TabR[i].r = LinePosition(GravInit,PointTwo,pPosTemp[i]);
            }
            qsort(TabR,LengthLocalLine,sizeof(STabIndice), CompareDouble);
            ...
            for (i=1;i<LengthLocalLine-1;i++)
            {
               ...
               {
                  Final= LineProjection(pPosTemp[TabRInv[k].i],pPosTemp[TabRInv[k+1].i],pPosInit[i],&Scal);
            ...
            if (pos != (*it)->end()) pos++;// on pointe sur le suivant
            for (i=1;i<LengthLocalLine-1;i++)
            {
               ...
               if (pos != (*it)->end()) *pos = pLocalLine[i];
               if (pos != (*it)->end()) pos++;// on pointe sur le suivant
            }
            if (pos != (*it)->end()) 
               for (i=1;i<LengthLocalLine/4-1;i++) // rewind the list but one if we are not at the end
                  pos --;
         } // if not fin
      } // while pos
   } // for all regions
