...
class CCurviDetect1D
{
   ...
   typedef AccumulatorDeduction<typename AryBase::value>         AccumulatorType;
...
public:
   ...
   void RespCurviMaxiFast(ImageSignalType * TabCoupe, IPoint2D *TabCoupeXY, double WidthPrior, utls::BAry *EdgeImage)
   {
      ...
      for (int IptFinal=(IptInitFixe+LargeurFourier+1); IptFinal<IptFinalMax; IptFinal++) //idem modif 21/11/2006
      {
      ...
      for (int IptFinal = IptInitFixe+LargeurFourier+1,i=0; IptFinal<IptFinalMax; IptFinal++,i++) // modif 21/11/2006 rajout pour limiter les surbalayage
      {
